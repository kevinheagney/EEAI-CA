...
#def get_input_data(place_holder_target_column) -> pd.DataFrame:
    ...
    # new
    # df["y"] = place_holder_target_column
    # df["yt3"] = df[Config.CLASS_COL3]

    # df["y"] = df[Config.CLASS_COL].str.cat(df[Config.CLASS_COL3], sep=' + ')
    df['y'] = df[Config.CLASS_COL] + ' ' + df[Config.CLASS_COL3] + ' ' + df[Config.CLASS_COL4]
    #
    # New
    #df["y"] = df[[df[Config.CLASS_COL],df[Config.CLASS_COL3]]]
    #df["yg2"] = df[Config.TYPE_COLS[0:2]]
    #df["yg3"] = df[Config.TYPE_COLS[0:3]]
    #df["yg1"] = df[Config.TYPE_COLS[0]]
    #df["yg2"] = pd.concat([df[Config.TYPE_COLS[0]], df[Config.TYPE_COLS[1]]])
    #df["yg3"] = pd.concat([df[Config.TYPE_COLS[0]], df[Config.TYPE_COLS[1]]], df[Config.TYPE_COLS[2]]])

    #df["yg1"] = df[Config.TYPE_COLS[0]]
    #df["y"] = ''.join((df[Config.TYPE_COLS[0:2]]))
    #df["yg3"] = df[Config.TYPE_COLS[0]], df[Config.TYPE_COLS[1]], df[Config.TYPE_COLS[2]]

    # original
    df = df.loc[(df["y"] != '') & (~df["y"].isna()),]
    # new
    #df = df.loc[(df["yg1"] != '') & (~df["yg1"].isna()),]
